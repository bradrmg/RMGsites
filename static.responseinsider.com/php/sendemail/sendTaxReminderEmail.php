...
if(($tourcode != "") && ($_REQUEST['Email'] != "") && ($_REQUEST['lead_type'] != "")){
	...
	$uid = $_REQUEST['TourCode'] . '~' . $todayTimeStamp . '~TaxReminder';
	if(!mssql_num_rows($queryemail)){
		...
		if(!mssql_num_rows($query)){
			...
			$tabDelimFileContent = $_REQUEST;
			//error_log("before: " . print_r($tabDelimFileContent));
			//unset($tabDelimFileContent["Speaker"]);
			//error_log("after: " . print_r($tabDelimFileContent));
			$vars .= "&" . http_build_query($_REQUEST);
			$vars .= "&uid=" . $uid . "~tax";
			$returnRslt = post_content("realestateeventregistration.a-rmgws01.blooint.com/script/sendemail/taxlien/reminderEmail.php", count($result), $vars);
...
$saveSql = "INSERT INTO LEADS.dbo.EmailHistory (TourCode, Speaker, Email, lead_type, Success, EmailUID) VALUES ('$tourcode', '$speaker', '$emailRequest', '$lead_type', '$successMsg', '$uid')";
mssql_query($saveSql);
