...
$selected = mssql_select_db($database, $dbhandle) or die("could not connect to database $database");
	...
	LEFT JOIN regString s ON r.ID = s.RMGregID
	where r.Registerdate >= '2014-08-10'
	and RMGType NOT IN ('PNA', 'FUTURE')
	and LeadType not in ('Online', 'LiveOps', 'XMAIL', 'OB', 'RMG', 'RMGRR', 'PNB')
	and p.error like '%inbound%'
	and Inbound_Number <> ''
...
$url =  'pl-rmgstaticws01.response.corp/php/bregpost.php';
